...
class NiveauxDAO
{
    ...
    public function getAllNiveaux() {
        $stmt = $this->pdo->prepare("SELECT * FROM 	niveau");
    ...
    public function getNiveaux($Id) {
        $stmt = $this->pdo->prepare("SELECT * FROM niveau WHERE Id = ?");
    ...
    {
        $sql = "INSERT INTO `niveau` (`Nom`, `Description`) VALUES (?, ?)";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$Niveaux->getNom(), $Niveaux->getDescription()]);

        header('location: ../index.php?success=addNiveauxsSuccess');

    }

    // Update Compenent
    public function updateNiveaux($dataCompenent) {
        $stmt = $this->pdo->prepare("UPDATE niveau
            SET Nom = ?,
            Description = ?
            WHERE Id = ?");
        ...
        $stmt->closeCursor();
        header("location: ../index.php?success=updateNiveauSuccess");
    }

    // Delete Compenent 
    public function DeleteNiveaux($NiveauxID)
    {
        $sql = "DELETE FROM niveau WHERE ID = ?";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$NiveauxID]);
   
        header("location: ../index.php?success=deleteNiveauSuccess");
    }
}