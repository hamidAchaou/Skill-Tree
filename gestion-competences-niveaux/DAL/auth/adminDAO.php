...
// include_once "../../Presentation/Admin/competences/index.php";
    class AdminDAO extends Dbh{
      ...
      protected function getAdmin($email, $password, $role) {
        ...
        if (!$stmt->execute(array($email))) {
            $stmt = null;
            header("location: ../../Presentation/Admin/auth/login.php?error=stmtfailed");
            exit();
        }
    
        if ($stmt->rowCount() == 0) {
            // $stmt = null;
            header("location: ../../Admin/auth/login.php?error=usernotfoundEmail");
            exit();
        }
        ...
        $checkPass = password_verify($password, $loginData[0]["Password"]);
          ...
          if ($checkPass == false) {
            ...
            $stmt = null;
            header("location: ../../Admin/auth/login.php?error=worningpassword");
            exit();
          } else
          if ($checkPass == true && $role == "admin") {
            ...
            if ($stmt->rowCount() == 0) {
              $stmt = null;
              header('location: ../../Presentation/Admin/auth/login.php?error=usernotfoundPass');
              exit();
            }
            ...
            $stmt = null;
            header('location: ../../Admin/competences/index.php?login=success');
